...
module.exports = {
  ...
  // Get TO offer products
  getTopOfferProducts:()=>{
    return new Promise(async (resolve,reject)=>{
      let offerProducts = await db.get().collection(collections.PRODUCT_COLLECTION).find({productVariants:{$elemMatch:{offer:{$gt:30}}}}).toArray()
...
checkWallet:(cash,userId)=>{
  ...
  get().collection(collections.USER_COLLECTION)
  .findOne({_id:objectId(userId),walletAmount: { $exists: true }})
